...
function SelectGroup({ className, ...props }: SelectPrimitive.Group.Props) {
	return (
		<SelectPrimitive.Group
			className={cn("scroll-my-1 space-y-0.5 p-1", className)}
			...
			>
				<SelectPrimitive.Popup
					className={cn(
						...
						"relative isolate z-50 max-h-(--available-height) w-(--anchor-width) min-w-36 origin-(--transform-origin) overflow-y-auto overflow-x-hidden rounded-md bg-background text-base-700 shadow-sm ring-1 ring-base-700/10 duration-100 data-[align-trigger=true]:animate-none data-closed:animate-out data-open:animate-in",
						className,
					)}
...
function SelectLabel({
	...
	return (
		<SelectPrimitive.GroupLabel
			className={cn("px-2 py-1 font-medium text-base-500 text-xs", className)}
	...
	return (
		...
		>
			<SelectPrimitive.ItemText
				className="flex flex-1 shrink-0 items-center gap-2 whitespace-nowrap pl-0.5 has-data-icon:pl-0 [&_svg:not([class*='size-'])]:size-3.5 [&_svg:not([class*='text-'])]:text-base-500"
				data-slot="item-text"
			>
				{children}
			</SelectPrimitive.ItemText>
			...
			>
				<CheckIcon className="pointer-events-none text-base-500" />
			</SelectPrimitive.ItemIndicator>
		</SelectPrimitive.Item>
	);
}
...
function SelectSeparator({
	...
	return (
		<SelectPrimitive.Separator
			className={cn("pointer-events-none -mx-1 my-1 h-px bg-base-200", className)}
