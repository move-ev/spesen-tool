...
function SidebarProvider({
	...
	const setOpen = React.useCallback(
		(value: boolean | ((value: boolean) => boolean)) => {
			...
			// biome-ignore lint/suspicious/noDocumentCookie: required for cookie to work
...
function Sidebar({
	...
	if (collapsible === "none") {
		return (
			<div
				className={cn(
					"flex h-full w-(--sidebar-width) flex-col bg-base-50 text-base-500",
					className,
				)}
				data-slot="sidebar"
				{...props}
	...
	if (isMobile) {
		return (
			<Sheet onOpenChange={setOpenMobile} open={openMobile} {...props}>
				<SheetContent
					className="w-(--sidebar-width) bg-base-50 p-0 text-sidebar-foreground [&>button]:hidden"
					data-mobile="true"
